"""Where a support may stand: the lattice reachability sweep.
...
shapely polygons. :mod:`rsupport.avoidance_polygon` does it the other way and is
still the default everywhere it can run — it is faster, and a shade less
conservative. This module exists for the one place it cannot run: a browser.

That backend's per-layer, per-radius chain of ``buffer`` → ``difference`` →
``intersection`` trips a GEOS overlay bug, ``TopologyException: depth
mismatch``, on the GEOS 3.12 that every Pyodide release ships — against the 3.13
a desktop shapely brings. Under Emscripten that C++ exception unwinds straight
past the interpreter: ``except BaseException`` does not catch it and the runtime
does not survive it, so there is no degrading gracefully and nothing to retry.
Neither dropping ``simplify`` nor ``shapely.set_precision`` avoids it; both were
tried. The browser needs a sweep that never calls those operations at all.
...
* ``dist[i]`` — millimetres from each cell to the nearest solid cell — is
  ...
  just ``dist[i] >= r + clearance``. The polygon backend pays for a separate
  buffer per radius per layer;
...
that over-marked set, thresholds carry a measured margin, and the dilation
radius is not rounded up. So the rasterised ``free`` is always a *subset* of the
...
asymmetry is why the self-printability invariant holds here too: measured across
every preset, violations are 0 under both backends, and the whole difference
between them lands on *dropped contacts* instead.
"""
...
class RasterRegion:
    ...
    def without(self, other: "RasterRegion") -> "RasterRegion":
        """This region minus `other` — the raster's set difference."""
        return RasterRegion(self.mask & ~other.mask, self.lattice)
...
class RasterAvoidanceField:
    ...
    def free(self, bucket: int, layer: int) -> RasterRegion:
    ...
    def contains(self, region: RasterRegion | None, xy) -> bool:
        if region is None or region.is_empty:
            return False
        return region.contains(xy)

    def nearest(self, region: RasterRegion, xy) -> np.ndarray:
